...
/* ---------------- AT32 的全局中断开关 ---------------- */
#define DISABLE_GLOBAL_INT __disable_irq();  // 关闭全局中断
#define ENABLE_GLOBAL_INT  __enable_irq();   // 开启全局中断

/* ---------------- AT32 的延时处理 ---------------- */
// 原代码里有很多 Nop() 用于等待 250ns。
// AT32 主频很高（比如288MHz），原来微秒级的 Nop() 会太快。
// 我们定义一个简单的空循环来稍微延时一下。
#define Nop()              { volatile int delay = 10; while(delay--); }
...
static UINT8 RxTxSpiData(UINT8 MosiByte)
{
    ...
    // 发送一个字节
    spi_i2s_data_transmit(SPI2, MosiByte);
...
UINT8 HW_Init(void)
{
    ...
    // 1. 这里不需要写 AT32 的初始化，AT32 的初始化请在 main 函数开头搞定
    // AT32_SPI1_Init(); 
    // AT32_GPIO_Init(); 
    
    // 2. 以下保留原来用于复位 ESC 中断屏蔽字的代码
    do
    {
        intMask = 0x93;
